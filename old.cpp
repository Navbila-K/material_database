// main.cpp
// Interactive material XML browser
...
static void show_metadata(const XmlNode &root) {
    ...
    if (!meta) {
        std::cout << "No <Metadata> section found.\n";
        return;
    }
    std::cout << "---- METADATA ----\n";
...
static void list_children_names(const XmlNode &node) {
    for (size_t i=0;i<node.children.size();++i) {
        std::cout << "  " << (i+1) << ". " << node.children[i].name << "\n";
    }
}

static const XmlNode* choose_child_by_number(const XmlNode &parent) {
    if (parent.children.empty()) {
        std::cout << "No children available.\n";
        return nullptr;
    }
    for (size_t i=0;i<parent.children.size();++i) {
        std::cout << "  " << (i+1) << ". " << parent.children[i].name << "\n";
    }
    std::cout << "  0. Back\n";
    int ch = ask_choice("Choose child number: ");
    if (ch <= 0) return nullptr;
    if (static_cast<size_t>(ch) > parent.children.size()) {
...
static void show_child_detail(const XmlNode &child) {
    ...
    // Detect if child contains <Entry> list
        ...
        for (const XmlNode* e : entries) {
            ...
            switch (c) {
                case 1:
                    ...
                    break;
                case 2:
                    std::cout << (unit.empty() ? "(none)" : unit);
                    break;
                case 3:
                    std::cout << (ref.empty() ? "(none)" : ref);
                    break;
                case 4:
                    std::cout << val;
...
static void show_node_children_as_fields(const XmlNode &node) {
    ...
    for (const auto &c : node.children) {
        ...
        if (!entries.empty()) {
            std::cout << c.name;
            if (c.attrs.count("unit"))
                std::cout << " [unit=" << c.attrs.at("unit") << "]";
            std::cout << ":\n";
...
int main() {
    try {
        ...
        int file_choice = -1;
        while (true) {
            file_choice = ask_choice("Choose file number to open (0=exit): ");
            if (file_choice == 0) { std::cout << "Exiting.\n"; return 0; }
            if (file_choice < 0 || static_cast<size_t>(file_choice) > files.size()) {
        ...
        MaterialDescriptor md = parseMaterialFile(chosen.string());
            ...
            while (!back_to_main) {
                ...
                if (top_choice == 2) {
                    ...
                    for (size_t i=0;i<property->children.size();++i) {
                        std::cout << "  " << (i+1) << ". " << property->children[i].name << "\n";
                    }
                    std::cout << "  0. Back\n";
                ...
                if (top_choice == 3) {
                    ...
                    for (size_t i=0;i<model->children.size();++i) {
                        std::cout << "  " << (i+1) << ". " << model->children[i].name << "\n";
                    }
                    std::cout << "  0. Back\n";
                    ...
                    if (pickedModel.name == "EOSModel") {
                                ...
                                if (eopt == 2) {
                                    ...
                                    while (!back_row) {
                                        ...
                                        for (size_t i=0;i<row->children.size();++i) {
                                            std::cout << "  " << (i+1) << ". " << row->children[i].name << "\n";
                                        }
                                        std::cout << "  0. Back\n";
                        ...
                        while (!back_model) {
                            ...
                            for (size_t i=0;i<pickedModel.children.size();++i) {
                                std::cout << "  " << (i+1) << ". " << pickedModel.children[i].name << "\n";
                            }
                            std::cout << "  0. Back\n";
